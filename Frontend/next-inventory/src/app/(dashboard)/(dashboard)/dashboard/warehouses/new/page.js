...
export default function WarehouseNew() {
  ...
  const handleSubmit = (e) => {
    ...
    }).then(() => { toast.success('Warehouse created'); router.push('/dashboard/warehouses'); }).catch((err) => toast.error(err.response?.data?.message || 'Failed')).finally(() => setLoading(false));
  };

  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="text-2xl font-bold text-slate-900">New Warehouse</h1><p className="mt-1 text-slate-600">Warehouse details</p>
      <form onSubmit={handleSubmit} className="mt-8 space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-card">
        ...
        <div className="flex gap-3 pt-4"><button type="submit" disabled={loading} className="rounded-lg bg-zoho-blue px-4 py-2 font-medium text-white hover:bg-zoho-dark-blue disabled:opacity-50">{loading ? 'Saving...' : 'Create'}</button><button type="button" onClick={() => router.push('/dashboard/warehouses')} className="rounded-lg border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50">Cancel</button></div>
      </form>
    </div>
  );
}
