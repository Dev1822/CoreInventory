...
export default function DeliveryNew() {
  ...
  useEffect(() => {
    Promise.all([productsAPI.getAll({ limit: 500 }), warehousesAPI.getAll()]).then(([pRes, wRes]) => {
      ...
      if (wRes.data.data.length > 0) setForm(f => ({ ...f, warehouse: wRes.data.data[0]._id }));
    });
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    }).then(() => { toast.success('Delivery created'); router.push('/dashboard/deliveries'); }).catch((err) => toast.error(err.response?.data?.message || 'Failed')).finally(() => setLoading(false));
  };

  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="text-2xl font-bold text-slate-900">New Delivery</h1><p className="mt-1 text-slate-600">Outgoing stock to customer</p>
      <form onSubmit={handleSubmit} className="mt-8 space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-card">
        ...
        <div className="flex gap-3 pt-4"><button type="submit" disabled={loading} className="rounded-lg bg-zoho-blue px-4 py-2 font-medium text-white hover:bg-zoho-dark-blue disabled:opacity-50">{loading ? 'Saving...' : 'Create'}</button><button type="button" onClick={() => router.push('/dashboard/deliveries')} className="rounded-lg border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50">Cancel</button></div>
      </form>
    </div>
  );
}
